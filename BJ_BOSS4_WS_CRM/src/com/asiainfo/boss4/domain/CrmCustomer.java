...
@Table(name = "CRM_CUSTOMER", schema = "ZG")
@Backup.CopyToSuffix(suffixes = {Backup.CopyToSuffix.HIS})
@CommonDao(fieldOfSelectConditions = { "custId"}, isOverwriteWhenAdd = true, dataStoreMode = DataStoreMode.DIRECT_UPDATE)
public class CrmCustomer extends BaseEntity implements java.io.Serializable {
	...
	/** minimal constructor */
	public CrmCustomer(Long custId, Byte custType, Byte gender,
	...
	/** full constructor */
	public CrmCustomer(Long custId, String custName, String custNameShort,
			Byte custType, Byte gender, Date birthday, Short occupation,
			Byte regType, String regNbr, Date regValidDate, Date regExpireDate,
			String regAddress, Integer vocation, Integer vocation2,
			Byte outFlag, String linker, String linkPhone, String fax,
			String postAddress, Integer postCode, String homeAddress,
			String company, String officePhone, String workAddress,
			String position, String workDept, Byte education, Byte marry,
			Byte vipFlag, String email, Short custClass, Integer credit,
			Integer custValue, Integer nationality, Integer provinceId,
			Short regionCode, Short countyCode, Integer regionDetail,
			Date createDate, Integer orgId, Date stsDate, Byte sts,
			Date validDate, Date expireDate, Integer busiCode,
			String custExtFlag, Long soNbr, String remark, Short relateClass,
			Byte vipStatus) {
		this.custId = custId;
	...
	// Property accessors
	@Id
	@Column(name = "CUST_ID", unique = true, nullable = false, precision = 14, scale = 0)
	...
	public BaseEntity modify(ServiceContext context) {
		CrmCustomer oldCust = (CrmCustomer)context.getBusinessDao().getEntity(this, this.getFieldOfSelectConditionsOnCommonDaoAnnotation());
		if (!DataValiditionHelper.isEquals(oldCust, this))
			return super.modify(context);
		else
			return this;
	}
	
	public static CrmCustomer queryByCa(ServiceContext context, String ca) {
		Criteria crit = context.getBusinessDao().getSess().createCriteria(CrmCustomer.class);
		
		try {
			@SuppressWarnings("unchecked")
			List<CrmCustomer> list = crit.add(Restrictions.eq("remark", ca)).setMaxResults(1).list();
