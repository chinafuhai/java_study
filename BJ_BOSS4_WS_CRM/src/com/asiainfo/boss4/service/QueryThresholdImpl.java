...
import com.asiainfo.boss4.support.Threshold;
import com.asiainfo.boss4.support.TranCode;

public class QueryThresholdImpl implements QueryBusinessService {
	...
	public String process(LazyDynaMap vo) {
		...
		try {
			...
			// 获取
			for (Map.Entry<Integer, String> m : Threshold.needNoticeThresholdMap().entrySet()) {
				retXml.append("<Threshold>")
				      .append("<threshold>").append(m.getKey()).append("</threshold>")
				      .append("<thresholdDesc>").append(m.getValue()).append("</thresholdDesc>")
				      .append("</Threshold>");
			}
